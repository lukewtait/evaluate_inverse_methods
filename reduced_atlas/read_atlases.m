...
T = readmatrix('vol_transform.txt') ; % transform
...
cmap = readmatrix('ROI_colors.txt') ; % get original colours from atlas printed in Glasser et al for reproducibility
colormap(cmap)

% Read surface atlas
surf = struct ; 
surf.vertices = readmatrix('surf_vertices.txt') ; 
surf.faces = readmatrix('surf_faces.txt') ; 
surfHCP = readmatrix('surf_HCP250.txt') ; 
figure(2), clf
